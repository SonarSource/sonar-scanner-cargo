...
The full configuration and troubleshooting reference for `cargo-sonar-scanner`. Start with the
[README](../README.md) for installation and a quick start.
...
quarantines one that a browser downloaded — see [troubleshooting](#a-downloaded-macos-binary-will-not-run).
